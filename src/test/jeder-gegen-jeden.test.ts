...
describe('Erstellen von Begegnungen', () => {
  ...
  const wettkampfGruppe = {
    id: 1,
    altersKlasse: Altersklasse.U11,
    gruppenGeschlecht: Geschlecht.m,
    teilnehmer: teilnehmer,
    gewichtsklasse: { name: "Gewichtskl.1", gewicht: 24.8 },
    name: "TestGruppenname"
  };

  it('korrekte ', () => {
    const erstellteBegegnungen = algorithmus.erstelleWettkampfGruppen(1, wettkampfGruppe, 10);
    
    erstellteBegegnungen[0].begegnungen.forEach(p => console.log(p.wettkaempfer1.name + "=>" + p.wettkaempfer2?.name));
    
    assert.notEqual(erstellteBegegnungen[0].begegnungen[0].wettkaempfer1.id, erstellteBegegnungen[0].begegnungen[1].wettkaempfer1.id)
  });
});